...
BASE_URL = "https://precize.in"

def get_all_share_urls():
    print("   Loading listing page for Precize...")
    urls = []
    with sync_playwright() as p:
        browser = p.chromium.launch(headless=True)
        page = browser.new_page()
        try:
            page.goto(f"{BASE_URL}/in/unlisted-shares", wait_until="networkidle", timeout=60000)
            time.sleep(3)
            ...
            for a in soup.find_all("a", href=True):
                href = a["href"]
                if "/in/unlisted-shares/" in href and href != "/in/unlisted-shares":
                    full_url = BASE_URL + href if href.startswith("/") else href
                    if full_url not in urls:
                        urls.append(full_url)
        except Exception as e:
            print("   Precize Listing Error:", e)
        browser.close()
    return urls

def scrape_detail(url, page, mode="all"):
    data = {"source": "precize", "extra_data": {}, "documents": []}
    slug = url.split("/in/unlisted-shares/")[-1].rstrip("/")
    data["slug"] = slug
...
def scrape_precize(mode="all", priority_slugs=None):
    if mode == "high-priority" and priority_slugs:
        urls = [f"{BASE_URL}/in/unlisted-shares/{s}" for s in priority_slugs]
    else:
        urls = get_all_share_urls()

    results = []
    print(f"   Found {len(urls)} URLs to scrape for Precize")
